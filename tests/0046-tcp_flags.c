...
#define NF5_RECORD(mtcp_flags)                                                 \
	{                                                                      \
		.srcaddr = NF5_IP(8, 8, 8, 8), /* Source IP Address */         \
				...
				.nexthop = NF5_IP(0, 0, 0, 0), /* Next hop     \
								  router's IP  \
								  Address */   \
				.input = 0,    /* Input interface index */     \
				.output = 255, /* Output interface index */    \
				.dPkts = constexpr_be16toh(1), /* Packets sent \
							  in Duration          \
							  (milliseconds        \
							  between 1st & last   \
							  packet in            \
							  this flow) */        \
				.dOctets = constexpr_be16toh(                  \
						70), /* Octets sent in         \
							Duration */            \
				.first = 0xa8484205, /* SysUptime at           \
							start of flow          \
							*/                     \
				.last = 0xa8484205,  /* and of last packet of  \
							the flow */            \
				.srcport = constexpr_be16toh(443), /* source   \
								      port     \
								      number   \
								      */       \
				.dstport = constexpr_be16toh(                  \
						10101),  /* destination port   \
							    number */          \
				.tcp_flags = mtcp_flags, /* Cumulative OR of   \
							    tcp flags */       \
				.proto = 2,    /* IP protocol, e.g., 6=TCP,    \
						  17=UDP, etc... */            \
				.tos = 0,      /* IP Type-of-Service */        \
				.src_as = 0,   /* source peer/origin           \
						  Autonomous System */         \
				.dst_as = 0,   /* dst peer/origin Autonomous   \
						  System */                    \
				.src_mask = 0, /* source route's mask bits */  \
				.dst_mask = 0, /* destination route's mask     \
						  bits */                      \
	}
...
static const NetFlow5Record record5 = {
		.flowHeader =
				{
						...
						.sys_uptime = constexpr_be16toh(
								12345),
						.unix_secs = constexpr_be16toh(
								12345),
						.unix_nsecs = constexpr_be16toh(
								12345),
						.flow_sequence = constexpr_be16toh(
								1050),
						.engine_type = 0, /* Type of
								     flow
								     switching
								     engine
								     (RP,VIP,etc.)*/
						.engine_id = 0,   /* Slot number
								     of the flow
								     switching
								     engine */
						.sampleRate = 0,  /* Packet
		...
		.flowRecord = {
				// Some random records
				NF5_RECORD(0x6c), NF5_RECORD(0xe0),
				NF5_RECORD(0x80), NF5_RECORD(0xee),
				NF5_RECORD(0x3c), NF5_RECORD(0xe5),
				NF5_RECORD(0x2e), NF5_RECORD(0xc6),
				NF5_RECORD(0xa8), NF5_RECORD(0x76),
				NF5_RECORD(0x6a), NF5_RECORD(0x9f),
				NF5_RECORD(0xc4), NF5_RECORD(0xa0),
				NF5_RECORD(0x64), NF5_RECORD(0x8a),
				NF5_RECORD(0xe3), NF5_RECORD(0x0b),
				NF5_RECORD(0x2a), NF5_RECORD(0xdc),
				NF5_RECORD(0xa5), NF5_RECORD(0x35),
				NF5_RECORD(0x7f), NF5_RECORD(0x1d),
				NF5_RECORD(0x65), NF5_RECORD(0x3b),
				NF5_RECORD(0x80), NF5_RECORD(0xd7),
				NF5_RECORD(0x8b), NF5_RECORD(0x89),
		}};
...
#define TEST_FLOW_HEADER                                                       \
	...
	.flow_sequence = constexpr_be32toh(12372811), .observation_id = 0
...
CHECKS_DEF(1, "CEU.....");
CHECKS_DEF(2, "C.......");
CHECKS_DEF(3, "CEU.PRS.");
CHECKS_DEF(4, "..UAPR..");
CHECKS_DEF(5, "CEU..R.F");
CHECKS_DEF(6, "..U.PRS.");
CHECKS_DEF(7, "CE...RS.");
CHECKS_DEF(8, "C.U.P...");
CHECKS_DEF(9, ".EUA.RS.");
CHECKS_DEF(10, ".EU.P.S.");
CHECKS_DEF(11, "C..APRSF");
CHECKS_DEF(12, "CE...R..");
CHECKS_DEF(13, "C.U.....");
CHECKS_DEF(14, ".EU..R..");
CHECKS_DEF(15, "C...P.S.");
CHECKS_DEF(16, "CEU...SF");
CHECKS_DEF(17, "....P.SF");
CHECKS_DEF(18, "..U.P.S.");
CHECKS_DEF(19, "CE.APR..");
CHECKS_DEF(20, "C.U..R.F");
CHECKS_DEF(21, "..UA.R.F");
CHECKS_DEF(22, ".EUAPRSF");
CHECKS_DEF(23, "...APR.F");
CHECKS_DEF(24, ".EU..R.F");
CHECKS_DEF(25, "..UAP.SF");
CHECKS_DEF(26, "C.......");
CHECKS_DEF(27, "CE.A.RSF");
CHECKS_DEF(28, "C...P.SF");
CHECKS_DEF(29, "C...P..F");
...
static int prepare_test_ipfix_tcpflags(void **state,
				       const void *v10_template,
				       size_t v10_template_len,
				       const void *v10_flow,
				       size_t v10_flow_len) {
	...
	static const struct checkdata checkdata = {
			.size = RD_ARRAYSIZE(ipfix_checks),
			.checks = ipfix_checks,
	};
...
static int prepare_test_ipfix_tcpflags_1b(void **state) {
	static const IPFIX_TEMPLATE(template,
				    TEST_FLOW_HEADER,
				    TEST_TEMPLATE_ID,
				    FLOW_ENTITIES_TCP_FLAGS_1B);
	static const IPFIX_FLOW(flow,
				TEST_FLOW_HEADER,
				TEST_TEMPLATE_ID,
				FLOW_ENTITIES_TCP_FLAGS_1B);
