...
#define FIXED_FLOW_ENTITIES(X)                                                 \
	...
	X(TRANSACTION_ID, 8, 0, UINT64_TO_UINT8_ARR(9179447921714724864))      \
...
#define V4_NO_NORMALIZE_CHECKDATA(t_direction,                                 \
				  lan_side_normalized_direction,               \
				  wan_side_normalized_direction,               \
				  t_src_mac,                                   \
				  t_dst_mac,                                   \
				  t_post_dst_mac,                              \
				  t_src_ip,                                    \
...
#define NORMALIZED_CHECKDATA(direction,                                        \
			     client_mac,                                       \
			     lan_ip,                                           \
			     lan_ip_net,                                       \
			     lan_ip_net_name,                                  \
...
#define CHECKDATA_NORMALIZE_upstream(t_src_mac,                                \
				     t_dst_mac,                                \
				     t_src_ip,                                 \
				     t_src_net,                                \
				     t_src_net_name,                           \
...
#define CHECKDATA_NORMALIZE_downstream(t_src_mac,                              \
				       t_dst_mac,                              \
				       t_src_ip,                               \
				       t_src_net,                              \
				       t_src_net_name,                         \
				       ...
				       t_src_port,                             \
				       dst_port)                               \
	NORMALIZED_CHECKDATA("downstream",                                     \
			     t_dst_mac,                                        \
			     t_dst_ip,                                         \
			     t_dst_net,                                        \
			     t_dst_net_name,                                   \
...
#define CHECKDATA_NORMALIZE_internal(t_src_mac,                                \
				     t_dst_mac,                                \
				     t_src_ip,                                 \
				     t_src_net,                                \
				     t_src_net_name,                           \
...
#define CHECKDATA_NO_SPAN_NORMALIZE_CHECKDATA(direction,                       \
					      ...
					      t_src_port,                      \
					      dst_port)                        \
	CHECKDATA_NORMALIZE_##direction(t_src_mac,                             \
					t_post_dst_mac,                        \
					t_src_ip,                              \
					t_src_net,                             \
					t_src_net_name,                        \
...
#define V4_LAN_NO_SPAN_NORMALIZE_CHECKDATA(t_direction,                        \
					   lan_normalized_direction,           \
					   wan_normalized_direction,           \
					   t_src_mac,                          \
					   t_dst_mac,                          \
					   t_post_dst_mac,                     \
					   t_src_ip,                           \
...
static int prepare_test_nf10_mac_direction_base(
		void **state,
...
#define TEST_TEMPLATE_FLOW(nf_dev_ip,                                          \
			   template,                                           \
			   template_size,                                      \
			   flow,                                               \
			   flow_size,                                          \
	...
	const struct test_params test_params[] = {
			// LAN SIDE: span true, span false, no span specified
			// (false)
			TEST_TEMPLATE_FLOW(
					0x04030201,
					&v10Template,
					...
					&v10Flow,
					sizeof(v10Flow),
					params->lan.checkdata_span_true_v4
							.checks,
					params->lan.checkdata_span_true_v4.size,
					.config_json_path = "./tests/"
							    "0015-"
							    "testMacDirection."
							    "json",
			...
			TEST_TEMPLATE_FLOW(0x04030301,
					   ...
					   &v10Flow,
					   sizeof(v10Flow),
					   params->lan.checkdata_span_false_v4
							   .checks,
					   params->lan.checkdata_span_false_v4
							   .size, ),
			...
			TEST_TEMPLATE_FLOW(0x04030201,
					   ...
					   &v10Flow_v6,
					   sizeof(v10Flow_v6),
					   params->lan.checkdata_span_true_v6
							   .checks,
					   params->lan.checkdata_span_true_v6
							   .size, ),
			TEST_TEMPLATE_FLOW(0x04030301,
					   ...
					   &v10Flow_v6,
					   sizeof(v10Flow_v6),
					   params->lan.checkdata_span_false_v6
							   .checks,
					   params->lan.checkdata_span_false_v6
							   .size, ),
			TEST_TEMPLATE_FLOW(0x04030401,
			...
			// WAN SIDE: span true, span false, no span specified
			// (false)
			// @todo delete ifs
			TEST_TEMPLATE_FLOW(0x04040201,
					   ...
					   &v10Flow,
					   sizeof(v10Flow),
					   params->wan.checkdata_span_true_v4
							   .checks,
					   params->wan.checkdata_span_true_v4
							   .size, ),
			TEST_TEMPLATE_FLOW(0x04040301,
					   ...
					   &v10Flow,
					   sizeof(v10Flow),
					   params->wan.checkdata_span_false_v4
							   .checks,
					   params->wan.checkdata_span_false_v4
							   .size, ),
			...
			TEST_TEMPLATE_FLOW(0x04040201,
					   ...
					   &v10Flow_v6,
					   sizeof(v10Flow_v6),
					   params->wan.checkdata_span_true_v6
							   .checks,
					   params->wan.checkdata_span_true_v6
							   .size, ),
			TEST_TEMPLATE_FLOW(0x04040301,
					   ...
					   &v10Flow_v6,
					   sizeof(v10Flow_v6),
					   params->wan.checkdata_span_false_v6
							   .checks,
					   params->wan.checkdata_span_false_v6
							   .size, ),
			TEST_TEMPLATE_FLOW(0x04040401,
...
static int prepare_test_nf10_dont_normalize(void **state) {
	static const struct test_mac_direction_params params = {
			...
			.lan.checkdata_span_true_v4.size = RD_ARRAYSIZE(
					checkdata_dont_normalize_ipv4),
			.lan.checkdata_span_false_v4.checks =
					checkdata_dont_normalize_ipv4,
			.lan.checkdata_span_false_v4.size = RD_ARRAYSIZE(
					checkdata_dont_normalize_ipv4),
			.lan.checkdata_span_true_v6.checks =
					checkdata_dont_normalize_ipv6,
			.lan.checkdata_span_true_v6.size = RD_ARRAYSIZE(
					checkdata_dont_normalize_ipv6),
			.lan.checkdata_span_false_v6.checks =
					checkdata_dont_normalize_ipv6,
			.lan.checkdata_span_false_v6.size = RD_ARRAYSIZE(
					checkdata_dont_normalize_ipv6),

			.wan.checkdata_span_true_v4.checks =
					checkdata_dont_normalize_ipv4,
			.wan.checkdata_span_true_v4.size = RD_ARRAYSIZE(
					checkdata_dont_normalize_ipv4),
			.wan.checkdata_span_false_v4.checks =
					checkdata_dont_normalize_ipv4,
			.wan.checkdata_span_false_v4.size = RD_ARRAYSIZE(
					checkdata_dont_normalize_ipv4),
			.wan.checkdata_span_true_v6.checks =
					checkdata_dont_normalize_ipv6,
			.wan.checkdata_span_true_v6.size = RD_ARRAYSIZE(
					checkdata_dont_normalize_ipv6),
			.wan.checkdata_span_false_v6.checks =
					checkdata_dont_normalize_ipv6,
			.wan.checkdata_span_false_v6.size = RD_ARRAYSIZE(
					checkdata_dont_normalize_ipv6),
	};
...
static int prepare_test_nf10_mac_direction(void **state) {
	static const struct test_mac_direction_params params = {
			...
			.wan.checkdata_span_false_v4.size = RD_ARRAYSIZE(
					wan_checkdata_span_false_v4),
			.wan.checkdata_span_true_v6.checks =
					wan_checkdata_span_true_v6,
			.wan.checkdata_span_true_v6.size = RD_ARRAYSIZE(
					wan_checkdata_span_true_v6),
			.wan.checkdata_span_false_v6.checks =
					wan_checkdata_span_false_v6,
			.wan.checkdata_span_false_v6.size = RD_ARRAYSIZE(
					wan_checkdata_span_false_v6),
