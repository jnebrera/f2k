		...
		.srcaddr = NF5_IP(8, 8, 8, 8), /* Source IP Address */         \
				...
				.dstport = constexpr_be16toh(                  \
						10101), /* destination port    \
							   number */           \
				.tcp_flags = 0, /* Cumulative OR of tcp        \
						   flags */                    \
...
static const NetFlow5Record record5 = {
		.flowHeader =
				{
						...
						.sys_uptime = constexpr_be32toh(
								600000),
						.unix_secs = constexpr_be32toh(
								1200),
						.unix_nsecs = constexpr_be32toh(
								12345),
		...
		.flowRecord = {
				NF5_RECORD(.dPkts = constexpr_be32toh(20),
...
#define NF_WLC_NO_TIME_ENTITIES(RT, R) NF9_ENTITIES_BASE(RT)

static const NF9_TEMPLATE(nf9_notime_template,
			  NF9_HEADER,
			  TEST_TEMPLATE_ID,
			  NF_WLC_NO_TIME_ENTITIES);
static const NF9_FLOW(nf9_notime_flow,
		      NF9_HEADER,
		      TEST_TEMPLATE_ID,
		      NF_WLC_NO_TIME_ENTITIES);
...
static const IPFIX_TEMPLATE(ipfix_uptime_template,
			    IPFIX_HEADER,
			    TEST_TEMPLATE_ID,
			    IPFIX_UPTIME_ENTITIES);
static const IPFIX_FLOW(ipfix_uptime_flow,
			IPFIX_HEADER,
			TEST_TEMPLATE_ID,
			IPFIX_UPTIME_ENTITIES);

static const IPFIX_TEMPLATE(ipfix_timestamp_template,
			    IPFIX_HEADER,
			    TEST_TEMPLATE_ID,
			    IPFIX_TIMESTAMP_ENTITIES);
static const IPFIX_FLOW(ipfix_timestamp_flow,
			IPFIX_HEADER,
			TEST_TEMPLATE_ID,
			IPFIX_TIMESTAMP_ENTITIES);
...
static int prepare_test_timestamp_split(void **state,
					const void *flow1,
					size_t flow1_size,
					const struct checkdata *checkdata1,
					size_t checkdata1_size,
					const void *flow2,
					size_t flow2_size,
					const struct checkdata *checkdata2,
					size_t checkdata2_size,
...
static int prepare_test_nf5_timestamp_split(void **state) {
	return prepare_test_nf5_base(state,
				     checkdata_split,
				     RD_ARRAYSIZE(checkdata_split),
				     true);
}

static int prepare_test_nf5_timestamp_dont_split(void **state) {
	return prepare_test_nf5_base(state,
				     checkdata_dont_split,
				     RD_ARRAYSIZE(checkdata_dont_split),
				     false);
}
...
static int prepare_test_ipfix_uptime_base(void **state,
					  ...
					  bool split_flow) {
	return prepare_test_timestamp_split(state,
					    &ipfix_uptime_template,
					    sizeof(ipfix_uptime_template),
					    NULL,
					    0,
					    &ipfix_uptime_flow,
					    sizeof(ipfix_uptime_flow),
					    checkdata,
					    checkdata_size,
					    split_flow);
}
...
static int prepare_test_ipfix_timestamp_uptime_dont_split(void **state) {
	return prepare_test_ipfix_uptime_base(
			state,
			ipfix_checkdata_dont_split,
			RD_ARRAYSIZE(ipfix_checkdata_dont_split),
			false);
}
...
static int prepare_test_ipfix_timestamp_uptime_split(void **state) {
	return prepare_test_ipfix_uptime_base(
			state,
			ipfix_checkdata_split,
			RD_ARRAYSIZE(ipfix_checkdata_split),
			true);
}
...
static int prepare_test_ipfix_timestamp_base(void **state,
					     ...
					     bool split_flow) {
	return prepare_test_timestamp_split(state,
					    &ipfix_timestamp_template,
					    sizeof(ipfix_timestamp_template),
					    NULL,
					    0,
					    &ipfix_timestamp_flow,
					    sizeof(ipfix_timestamp_flow),
					    checkdata,
					    checkdata_size,
					    split_flow);
}
...
static int prepare_test_ipfix_timestamp_dont_split(void **state) {
	return prepare_test_ipfix_timestamp_base(
			state,
			ipfix_checkdata_dont_split,
			RD_ARRAYSIZE(ipfix_checkdata_dont_split),
			false);
...
static int prepare_test_nf9_no_timestamp_base(void **state, bool split) {
	...
	return prepare_test_timestamp_split(state,
					    &nf9_notime_template,
					    sizeof(nf9_notime_template),
					    NULL,
					    0,
					    &nf9_notime_flow,
					    sizeof(nf9_notime_flow),
					    &checkdata_notime,
					    1,
					    split);
}
