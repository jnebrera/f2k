...
static const struct {
	...
	int try_json;
		...
		{.src = "hello", .expected_dst = "hello", .try_json = 1},
...
static void
assert_escape_fn(const char *src, const char *expected_result, int try_json) {
...
static void test_utf8(void **state) {
	...
	for (i = 0; i < RD_ARRAY_SIZE(test_cases); ++i) {
		assert_escape_fn(test_cases[i].src,
				 test_cases[i].expected_dst,
				 test_cases[i].try_json);
	}
}

int main() {
	static const struct CMUnitTest tests[] = {cmocka_unit_test(test_utf8)};
