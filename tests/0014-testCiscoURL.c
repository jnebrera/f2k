...
#define TEST_IPFIX_FLOW_HEADER                                                 \
	.unix_secs = 0xdd5d6952, .flow_sequence = 0x38040000,                  \
...
#define CISCO_DPI_FIELD(ID, ...)                                               \
	CISCO_DPI_LEN(ARGS(__VA_ARGS__)), ID, __VA_ARGS__
...
#define T_CISCO_URL                                                            \
	CISCO_HTTP_URL(                                                        \
		'/', 'p', 'r', 'o', 'f', 'i', 'l', 'e', 's', '/', 'p', 'r',    \
		'o', 'f', 'i', 'l', 'e', '_', '1', '2', '3', '4', '5', '6',    \
		'7', '7', '_', '7', '5', 's', 'q', '_', '1', '1', '2', '3',    \
		'4','5','6','7','8','2','.','j','p','g')
...
#define CISCO_L2_HOST_HU                                                       \
	CISCO_HTTP_HOST(HTTP_PROTO, CISCO_L2_HOST0, EXAMPLE_URL)
#define CISCO_L2_HOST_HSU                                                      \
	CISCO_HTTP_HOST(HTTPS_PROTO, CISCO_L2_HOST0, EXAMPLE_URL)
...
#define T_CISCO_REFERER_HU                                                     \
	CISCO_HTTP_REFERER(HTTP_PROTO, T_CISCO_REFERER0, EXAMPLE_URL)
#define T_CISCO_REFERER_HSU                                                    \
	CISCO_HTTP_REFERER(HTTPS_PROTO, T_CISCO_REFERER0, EXAMPLE_URL)

#define T_CISCO_SSL_CN0                                                        \
	'w', 'w', 'w', '.', 'e', 'x', 'a', 'm', 'p', 'l', 'e', '.', 'c', 'o',  \
			'm'
...
#define T_CISCO_SSL_CN_HU CISCO_SSL_CN(HTTP_PROTO, T_CISCO_SSL_CN0, EXAMPLE_URL)
#define T_CISCO_SSL_CN_HSU                                                     \
	CISCO_SSL_CN(HTTPS_PROTO, T_CISCO_SSL_CN0, EXAMPLE_URL)
...
#define CISCO_IPv6_REFERER_HS CISCO_HTTP_REFERER(HTTPS_PROTO, IPv6_HTTP_ADDR)
#define CISCO_IPv6_REFERER_U CISCO_HTTP_REFERER(IPv6_HTTP_ADDR, EXAMPLE_URL)
#define CISCO_IPv6_REFERER_HU                                                  \
	CISCO_HTTP_REFERER(HTTP_PROTO, IPv6_HTTP_ADDR, EXAMPLE_URL)
#define CISCO_IPv6_REFERER_HSU                                                 \
	CISCO_HTTP_REFERER(HTTPS_PROTO, IPv6_HTTP_ADDR, EXAMPLE_URL)
	...
	{                                                                      \
		{.key = "host", .value = PRE "instagram.com" POST},            \
				{.key = "host_l2_domain",                      \
				 .value = "instagram.com"},                    \
				{.key = "referer",                             \
				 .value = PRE "www.elmundo.es" POST},          \
				{.key = "referer_l2", .value = "elmundo.es"},  \
				{.key = "https_common_name",                   \
				 .value = PRE "www.example.com" POST},         \
				{.key = "http_host",                           \
				 .value = PRE "instagram.com" POST},           \
				{.key = "http_host_l2",                        \
				 .value = "instagram.com"},                    \
				{.key = "http_referer",                        \
				 .value = PRE "www.elmundo.es" POST},          \
...
#define PROTO_IP_CHECKDATA(PRE, POST)                                          \
	{                                                                      \
		{.key = "host", .value = PRE TEST_EXPECTED_IPv4 POST},         \
				{.key = "host_l2_domain",                      \
				 .value = TEST_EXPECTED_IPv4},                 \
				{.key = "referer",                             \
				 .value = PRE TEST_EXPECTED_IPv6 POST},        \
				{.key = "referer_l2",                          \
				 .value = TEST_EXPECTED_IPv6},                 \
				{.key = "https_common_name", .value = NULL},   \
				{.key = "http_host",                           \
				 .value = PRE TEST_EXPECTED_IPv4 POST},        \
				{.key = "http_host_l2",                        \
				 .value = TEST_EXPECTED_IPv4},                 \
				{.key = "http_referer",                        \
				 .value = PRE TEST_EXPECTED_IPv6 POST},        \
...
static int prepare_test_nf10_cisco_url(void **state) {
	...
	static const IPFIX_FLOW(l2d_v10Flow,
				TEST_IPFIX_FLOW_HEADER,
				IPFIX_TEMPLATE_ID,
				L2_ENTITIES);
	static const IPFIX_FLOW(hrd_flow,
				TEST_IPFIX_FLOW_HEADER,
				IPFIX_TEMPLATE_ID,
				HOST_REFERER_ENTITIES);
	static const IPFIX_FLOW(https_url_flow,
				TEST_IPFIX_FLOW_HEADER,
				IPFIX_TEMPLATE_ID,
				PROTO_URL_ENTITIES);
	static const IPFIX_FLOW(ip_flow,
				TEST_IPFIX_FLOW_HEADER,
				IPFIX_TEMPLATE_ID,
				PROTO_IP_ENTITIES);

#define CHECK(checkdata)                                                       \
	{ .checks = checkdata, .size = RD_ARRAYSIZE(checkdata) }
