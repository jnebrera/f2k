...
/// Test a V5 flow
#define TEST_V5(t_netflow_src_ip,                                              \
		t_record,                                                      \
		t_record_size,                                                 \
		t_checkdata,                                                   \
		t_checkdata_size,                                              \
		...)                                                           \
	{                                                                      \
		.netflow_src_ip = t_netflow_src_ip, .record = t_record,        \
		.record_size = t_record_size, .checkdata = t_checkdata,        \
		.checkdata_size = t_checkdata_size, __VA_ARGS__                \
	}

// Test V9/IPFIX template+flow
#define TEST(t_netflow_src_ip,                                                 \
	     t_template,                                                       \
	     t_template_size,                                                  \
	     t_record,                                                         \
	     t_record_size,                                                    \
...
static int prepare_test_v5_ipfix_geoip(void **state) {
	...
	return prepare_tests_v5_ipfix_record(
			state, checkdata, RD_ARRAYSIZE(checkdata), false);
}
...
static int prepare_test_v5_ipfix_normalized(void **state) {
	...
	return prepare_tests_v5_ipfix_record(
			state, checkdata, RD_ARRAYSIZE(checkdata), true);
}
