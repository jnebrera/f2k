...
function docker_run () {
        ...
        shift
        docker run $DOCKER_COMMON_ARGS $docker_extra_args f2k-dev $*
}
...
function run_f2k_tests () {
        ...
        git clean -fqx --exclude=tests/asn*.dat --exclude=tests/country*.dat
        mkdir -p $output_dir
        docker_run "" "./configure $coverage_arg $*"
        docker_run "" "make all"
        docker_run "-v $output_dir:$output_dir -e TEST_REPORTS_DIR=$output_dir --link kafka" "make tests"

        if [[ ! -z "$coverage_arg" ]]; then
                docker_run "--link kafka -v $output_dir:$output_dir -e TEST_REPORTS_DIR=$output_dir -e COVERAGE_OUTPUT_DIRECTORY=\"$output_dir/coverage\"" "make coverage"
