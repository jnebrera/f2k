...
#define NF5_RECORD_ADDR(...)                                                   \
	{                                                                      \
		.srcaddr = NF5_IP(10, 13, 0, 1),                               \
		.dstaddr = NF5_IP(10, 14, 0, 1),                               \
		.input = constexpr_be16toh(0),                                 \
		.output = constexpr_be16toh(255), .first = 0xa8484205,         \
		.last = 0xa8484205, .srcport = constexpr_be16toh(443),         \
		.dstport = constexpr_be16toh(10101), __VA_ARGS__               \
	}
...
#define NF5_RECORD(mengine_type, mengine_id, ...)                              \
	{                                                                      \
		.flowHeader = {NF5_RECORD_HEADER_DEFAULTS,                     \
			       ...
			       .engine_type = mengine_type,                    \
			       .engine_id = mengine_id},                       \
		.flowRecord = {                                                \
			NF5_RECORD_ADDR(__VA_ARGS__)                           \
		}                                                              \
	}

static const NetFlow5Record record_obs_id_1 =
		NF5_RECORD(0,
			   1,
			   .dPkts = constexpr_be32toh(1),
			   .dOctets = constexpr_be32toh(88));
static const NetFlow5Record record_obs_id_2 =
		NF5_RECORD(0,
			   2,
...
static const IPFIX_TEMPLATE(ipfix_template_oid_1,
			    TEST_FLOW_HEADER(1),
			    TEST_TEMPLATE_ID,
			    FLOW_ENTITIES_OBSERVATION_ID_1);
static const IPFIX_TEMPLATE(ipfix_template_oid_2,
			    TEST_FLOW_HEADER(2),
			    TEST_TEMPLATE_ID,
