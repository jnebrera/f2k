...
static void init_test_kafka_consumer(const char *brokers, const char *topic) {
	...
	if (!(test_consumer_rk = rd_kafka_new(RD_KAFKA_CONSUMER,
					      conf,
					      errstr,
					      sizeof(errstr)))) {
		fprintf(stderr,
...
static void init_test_kafka_producer(const char *brokers, const char *topic) {
	...
	if (!(test_producer_rk = rd_kafka_new(RD_KAFKA_PRODUCER,
					      rk_conf,
					      errstr,
					      sizeof(errstr)))) {
		fprintf(stderr,
	...
	if (test_producer_rk != NULL) {
		test_producer_rkt = rd_kafka_topic_new(
				test_producer_rk, topic, rkt_conf);
...
static void init_kafka_producer(const char *broker_ip, const char *topic) {
	...
	if (!(readOnlyGlobals.kafka.rk = rd_kafka_new(RD_KAFKA_PRODUCER,
						      rk_conf,
						      errstr,
						      sizeof(errstr)))) {
		fprintf(stderr,
	...
	if (readOnlyGlobals.kafka.rk != NULL) {
		readOnlyGlobals.kafka.rkt = rd_kafka_topic_new(
				readOnlyGlobals.kafka.rk, topic, rkt_conf);
...
int nf_test_setup(void **state) {
	(void)state;
	/// @TODO we have, to get away of this stuff!! the code is too dependent
	/// of itself
	readWriteGlobals = calloc(1, sizeof(readWriteGlobals[0]));

	readOnlyGlobals.dontReforgeFarTimestamp =
			1; /* We can't test if this is not setted */
	return 0;
}
...
prepare_tests(const struct test_params *test_params, size_t test_params_size) {
	...
	rd_calloc_struct(&st,
			 sizeof(*st),
			 test_params_size * sizeof(test_params[0]),
			 test_params,
			 &st->params.records,
			 sizeof(sl),
			 sl,
...
static int load_geoip_databases(const char *geoip_path) {
	...
	if (!config_file) {
		...
		void *unpack_private = rb_json_assert_unpack(config_file,
							     0,
							     "{s:s,s:s}",
							     "as-path",
							     &AS_path,
...
test_flow_i(const struct test_params *params, worker_t *worker) {
	...
	if (params->config_json_path) {
		if (readOnlyGlobals.rb_databases.sensors_info) {
			delete_rb_sensors_db(readOnlyGlobals.rb_databases
							     .sensors_info);
		}
	...
	if (params->mac_vendor_database_path) {
		if (readOnlyGlobals.rb_databases.mac_vendor_database_path) {
			free(readOnlyGlobals.rb_databases
					     .mac_vendor_database_path);
		}
	...
	if (params->dns_servers) {
		...
		size_t i = 0;
		readOnlyGlobals.udns.csv_dns_servers =
				strdup(params->dns_servers);
		...
		for (i = 0; NULL != readOnlyGlobals.udns.dns_poll_threads &&
			...
			struct rb_dns_info *info =
					&readOnlyGlobals.udns.dns_info_array[i];
			...
			if (NULL == info->dns_ctx) {
				traceEvent(TRACE_ERROR,
					   "Can't allocate DNS context %zu "
					   "info",
					   i);
			}

			const int thread_create_rc = rd_thread_create(
					&readOnlyGlobals.udns
							 .dns_poll_threads[i],
					thread_name,
					attr,
			...
			if (thread_create_rc < 0) {
				...
				traceEvent(TRACE_ERROR,
					   "Can't allocate DNS polling thread "
					   "%zu: %s",
					   i,
					   errstr);
			}
	...
	if (params->dns_servers) {
		...
		readOnlyGlobals.udns.cache = dns_cache_new(dns_cache_size_m,
							   dns_cache_timeout_s);
	}
	...
	if (test_producer_rkt) {
		...
		if (rd_kafka_produce(test_producer_rkt,
				     0,
				     RD_KAFKA_MSG_F_COPY,
				     buf,
				     params->record_size,
				     &params->netflow_src_ip,
				     sizeof(uint32_t),
				     NULL) == -1) {
			fprintf(stderr,
				"%% Failed to produce to topic %s "
				"partition %i: %s\n",
				rd_kafka_topic_name(test_producer_rkt),
				0,
		...
		if (sensor_object) {
			...
			while (true) {
				pthread_mutex_lock(&worker->templates_queue
								    .rfq_lock);
				const int cnt = worker->templates_queue.rfq_cnt;
				pthread_mutex_unlock(
						&worker->templates_queue
								 .rfq_lock);
			...
			mem_wraps_set_fail_in(mem_stash); // fail beyond this
							  // point
			struct string_list *ret =
					dissectNetFlow(worker,
						       sensor_object,
						       params->netflow_src_ip,
						       params->record,
						       params->record_size);
...
static void check_string_list(struct string_list *sl,
	...
	for (i = 0, iter = sl; i < checkdata_size && iter;
	     ++i, iter = iter->next) {
		rb_assert_json(iter->string->buf,
			       iter->string->bpos,
			       &checkdata[i]);
	}
...
void testFlow(void **state) {
	...
	if (st->params.records->kafka_test_producer_url) {
		init_test_kafka_producer(
				st->params.records->kafka_test_producer_url,
				input_topic);
		if (NULL == test_producer_rkt) {
			printf("TEST Producer: Can't connect to Kafka "
			       "broker\n");
			exit(1);
		}
	}
	...
	if (st->params.records->kafka_consumer_url) {
		...
		for (i = 0; i < RD_ARRAYSIZE(props); ++i) {
			char errstr[512];
			const int set_rc = rd_kafka_conf_set(
	...
	if (st->params.records->kafka_producer_url) {
		init_kafka_producer(st->params.records->kafka_producer_url,
				    output_topic);
	...
	if (st->params.records->kafka_test_consumer_url) {
		init_test_kafka_consumer(
				st->params.records->kafka_test_consumer_url,
				output_topic);
		if (NULL == test_consumer_rk) {
			printf("TEST Consumer: Can't connect to Kafka "
			       "broker\n");
			exit(1);
		}
	}
	...
	if (test_consumer_rk) {
		...
		while (true) {
			rkmessage = rd_kafka_consumer_poll(test_consumer_rk,
							   100);
			if (rkmessage) {
		...
		while (RD_KAFKA_RESP_ERR_NO_ERROR !=
		       rd_kafka_flush(readOnlyGlobals.kafka.rk, 20))
			;
		...
		while (1) {
			...
			if (rkmessage &&
				...
				st->ret.sl[1] = (struct string_list *)calloc(
						1, sizeof(struct string_list));
				st->ret.sl[1]->string = (struct printbuf *)
						calloc(1,
						       sizeof(struct printbuf));
				st->ret.sl[1]->string->buf = (char *)calloc(
						rkmessage->len, sizeof(char));
				st->ret.sl[1]->string->bpos = rkmessage->len;
				st->ret.sl[1]->string->size = rkmessage->len;
				memcpy(st->ret.sl[1]->string->buf,
				       rkmessage->payload,
				       rkmessage->len);
				rd_kafka_message_destroy(rkmessage);
				break;
		...
		for (i = 0; i < st->params.records_size; ++i) {
			st->ret.sl[i] = test_flow_i(&st->params.records[i],
						    worker);
		}
	}
