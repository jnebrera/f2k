...
#define NF9_CHECKDATA(T_BYTES, T_PKTS)                                                \
	{                                                                             \
		{.key = "type", .value = "netflowv9"},                                \
				{.key = "client_mac",                                 \
				 .value = "b8:17:c2:28:b0:c7"},                       \
				{.key = "lan_ip",                                     \
				 .value = "10.13.94.223"}, /* {.key="application_id", \
					.value="13:453"},                             \
					*/                                            \
				{.key = "wireless_id", .value = "local-wifi"},        \
				{.key = "direction", .value = "ingress"},             \
				{.key = "sensor_ip", .value = "4.3.2.1"},             \
				{.key = "bytes", .value = T_BYTES}, {                 \
			.key = "pkts", .value = T_PKTS                                \
		}                                                                     \
	}

static int test_nf9_0(void **state,
		      const void *template,
...
#define NF9_ENTITIES(RT, R) NF9_ENTITIES_BASE(RT, 7603, 263)
	...
	static const NF9_TEMPLATE(
			v9Template, FLOW_HEADER, TEMPLATE_ID, NF9_ENTITIES);
...
#define CHECKS(values)                                                         \
	{ .size = RD_ARRAYSIZE(values), .checks = values }
	static const struct checkdata checkdata[] = {
			CHECKS(checkdata_values_1), CHECKS(checkdata_values_2),
	};
#undef CHECKS
...
#define NF9_ENTITIES(RT, R) NF9_ENTITIES_BASE(RT, 7603, 263)
	static const NF9_TEMPLATE_FLOW(
			flow, FLOW_HEADER, TEMPLATE_ID, NF9_ENTITIES);
